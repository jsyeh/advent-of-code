...
def findAns2(scene2, oldAns):
    M, N = len(scene2), len(scene2[0])
    for i in range(M):
        for j in range(N):
            scene2[i][j] = inv[scene2[i][j]] # 挑一個，變異
            now = findAns(scene2, oldAns)
            if now!=-1 and now != oldAns: 
                #print(now)
                #print(scene2)
                return now
            scene2[i][j] = inv[scene2[i][j]] # 還原
    print(scene)
    print(scene2)
    # 糟，如果沒有 return 就完蛋了
...
print('====Part 2====')
# 可能用暴力法，再把每一個點都弄髒，再看是否有解
print(ans2) # Part 2 我的 Puzzle Input 對應答案 36735
