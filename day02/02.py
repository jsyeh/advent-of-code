...
for line in lines:
    ...
    for i in range(2,len(a), 2):
        n = int(a[i])
        if a[i+1].find('red')==0: r = max(r,n)
        if a[i+1].find('green')==0: g = max(g,n)
        if a[i+1].find('blue')==0: b = max(b,n)
    ans += r*g*b
print(ans) # Part 2 我的 Puzzle Input 對應答案 70924
