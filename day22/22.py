...
# 和剛剛很像, 反過來,看能掉下多少
ans = 0
#print(len(mustHave), mustHave)
for i in range(N):
    if i in mustHave:
        # 接下來要進行 chain 連鎖消掉的任務
        thisSet = set() # 裡面放會影響到的上方的方塊編號
        thisSet.add(i)
        queue = deque()
        queue.append(i)
        while len(queue)>0:
            now = queue.popleft()
            for up in aboves[now]:
                if up in thisSet: continue # 用過就不要用
                if len(set(below[up])-thisSet)==0:
                    queue.append(up)
                    thisSet.add(up)
        ans += len(thisSet)-1 # 要扣掉自己
print(ans) # Part 2 我的 Puzzle Input 對應答案 71002
# 140423 is too high 我知道了，不是所有 above 的brick 都會掉 --- 
# 因為有些還有人撐著, 所以 support 不見時，要再看有沒有其他 below 
