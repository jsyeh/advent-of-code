...
#""" # 下面是簡單的 Sample Input 測資，在 Part 1 會輸出 3749，在 Part 2 會輸出 11387
lines = '''190: 10 19
3267: 81 40 27
83: 17 5
156: 15 6
...
# Puzzle Input 在 Part1 會輸出 66343330034722，在 Part 2 會輸出 637696070419031
...
# 新的 helper()函式，要多做「數字變字串、加起來」
def helper(target, ans, arr, i, N):
    if i==N:
        if target==ans: return target
        else: return 0
    ans1 = helper(target, ans+int(arr[i]), arr, i+1, N)
    ...
    ans3 = helper(target, int(str(ans)+arr[i]), arr, i+1, N)
    return ans3  # 多了這一項，把「數字」像「字串」一樣接起來
...
print(ans) # Part 2 我的 Puzzle Input 對應答案 637696070419031
